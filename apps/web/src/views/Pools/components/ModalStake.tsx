      ...
      <StyledModalContainer>
        ...
        <ContentContainer>
          ...
          <GetLP className="get_lp">
            <a
              href={`/add/${XOX_ADDRESS[chainId]}/${
                secondToken.address ? secondToken.address : NATIVE[chainId].symbol
              }?step=1&chainId=${chainId}`}
              target="_blank"
              rel="noreferrer"
            >
              <p style={{ display: 'flex', alignItems: 'center' }}>
                <span>{t('Get %symbol%', { symbol: `XOX - ${secondToken?.symbol} LP` })}</span>
